...
snp = 
  ...
  separate(date, into = c("year","month", "day"), convert = TRUE) %>%      mutate(month = as.integer(month),
        month = month.name[month],
        year = as.integer(year),
        day = as.integer(day)) %>% 
  select(-day) %>% 
...
In the `pols_month` dataset, there are `r nrow(pols_month)` observations and `r ncol(pols_month)` variables. 
In the `snp` dataset, there are `r nrow(snp)` observations and `r ncol(snp)` variables.
The resulting dataset, `join`, contains `r nrow(join)` observations and `r ncol(join)` variables. The dimension of the resulting dataset is `r dim(join)`. The range of years is between `r min(join$year)` and `r max(join$year)`. The variables in the dataset include `president`, `unemployment_rate`, and `close`. 
